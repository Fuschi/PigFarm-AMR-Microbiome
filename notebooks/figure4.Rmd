...
p_arg_contigs_sp <- arg_contigs %>%
  group_by(sample_id, species_kraken2023) %>%
  summarise(abun = sum(`RPKM coverm`), .groups = "drop") %>%
  group_by(sample_id) %>%
  mutate(rela = abun/sum(abun)) %>%
  ungroup() %>%
  left_join(meta_sample, by = "sample_id") %>%
  mutate(species_fct = if_else(species_kraken2023 %in% names(color_species), species_kraken2023, "Others")) %>%
...
p_arg_contigs_gen <- arg_contigs %>%
  group_by(sample_id, genus_kraken2023) %>%
  summarise(abun = sum(`RPKM coverm`), .groups = "drop") %>%
  group_by(sample_id) %>%
  mutate(rela = abun/sum(abun)) %>%
  ungroup() %>%
  left_join(meta_sample, by = "sample_id") %>%
  mutate(genus_fct = if_else(genus_kraken2023 %in% names(color_genus), genus_kraken2023, "Others")) %>%
  # Commentare anche le 3 righe sotto se vuoi anche le contigs sconosciute
...
p_arg_contigs_fam <- arg_contigs %>%
  group_by(sample_id, family_kraken2023) %>%
  summarise(abun = sum(`RPKM coverm`), .groups = "drop") %>%
  group_by(sample_id) %>%
  mutate(rela = abun/sum(abun)) %>%
  ungroup() %>%
  left_join(meta_sample, by = "sample_id") %>%
  mutate(family_fct = if_else(family_kraken2023 %in% names(color_family), family_kraken2023, "Others")) %>%
  # Commentare anche le 3 righe sotto se vuoi anche le contigs sconosciute
...
p_arg_contigs_phy <- arg_contigs %>%
  group_by(sample_id, phylum_kraken2023) %>%
  summarise(`RPKM coverm` = sum(`RPKM coverm`), .groups = "drop") %>%
  group_by(sample_id) %>%
  mutate(rela = `RPKM coverm`/sum(`RPKM coverm`)) %>%
  ungroup() %>%
  left_join(meta_sample, by = "sample_id") %>%
  mutate(phylum_fct = if_else(phylum_kraken2023 %in% names(color_phylum), phylum_kraken2023, "Others")) %>%
  # Commentare anche le 3 righe sotto se vuoi anche le contigs sconosciute
