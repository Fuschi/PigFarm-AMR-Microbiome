...
abun_arg_contigs_antFam <- arg_contigs %>%
  dplyr::select(sample_id, Antibiotic_Family, `RPKM coverm`) %>%
  group_by(sample_id, Antibiotic_Family) %>%
...
p_corr_glm <- df_corr_glm_filt %>%
  ...
  mutate(amr_fam = case_when(
    ...
    amr_fam == "Oxazolidinone, Amphenicol, Lincosamide, Streptogramin A, Pleuromutilin" ~
      "Oxazolidinone, Amphenicol, Lincosamide,\nStreptogramin A, Pleuromutilin",
    TRUE ~ amr_fam
  )) %>%
...
p_amr_kraken <- arg_contigs %>%
  select(Contig_name, genus_kraken2023, `RPKM coverm`) %>%
  mutate(genus_kraken = fct_lump_min(genus_kraken2023, 2),
         genus_kraken = fct_infreq(genus_kraken),
         genus_kraken = fct_relevel(genus_kraken, "Other", after = Inf)) %>%
  ggplot(aes(x = genus_kraken)) +
  geom_bar() +
...
```{r}
png(filename = "../plots/amr_kraken.png", width = 12*300, height = 6*300, res = 300)
p_amr_kraken
dev.off()
```
