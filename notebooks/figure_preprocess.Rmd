...
bac_reads_sp <- bac_reads %>%
  ...
  mutate(highlight = any(rela >= .1), .by = "species")

bac_reads_gen <- bac_reads_sp %>%
  group_by(sample_id, genus, Surface) %>%
  summarise(rela = sum(rela), .groups = "drop") %>%
  mutate(highlight = any(rela >= .1), .by = genus)

bac_reads_fam <- bac_reads_sp %>%
  group_by(sample_id, family, Surface) %>%
  summarise(rela = sum(rela), .groups = "drop") %>%
  mutate(highlight = any(rela >= .1), .by = family)

bac_reads_phy <- bac_reads_sp %>%
  group_by(sample_id, phylum, Surface) %>%
  summarise(rela = sum(rela), .groups = "drop") %>%
  mutate(highlight = any(rela >= .1), .by = phylum) 
```

```{r}
arg_contigs_sp <- arg_contigs %>%
  #filter(species_diamond != "Unknown") %>%
  group_by(sample_id, species_kraken2023, Surface) %>%
  summarise(abun = sum(`RPKM coverm`), .groups = "drop") %>%
  mutate(rela = abun / sum(abun), .by = "sample_id") %>%
  mutate(highlight = any(rela >= .1), .by = species_kraken2023)

arg_contigs_gen <- arg_contigs %>%
  group_by(sample_id, genus_kraken2023, Surface) %>%
  summarise(abun = sum(`RPKM coverm`), .groups = "drop") %>%
  mutate(rela = abun / sum(abun), .by = "sample_id") %>%
  mutate(highlight = any(rela >= .1), .by = genus_kraken2023)
    
arg_contigs_fam <- arg_contigs %>%
  group_by(sample_id, family_kraken2023, Surface) %>%
  summarise(abun = sum(`RPKM coverm`), .groups = "drop") %>%
  mutate(rela = abun / sum(abun), .by = "sample_id") %>%
  mutate(highlight = any(rela >= .1), .by = family_kraken2023)

arg_contigs_phy <- arg_contigs %>%
  group_by(sample_id, phylum_kraken2023, Surface) %>%
  summarise(abun = sum(`RPKM coverm`), .groups = "drop") %>%
  mutate(rela = abun / sum(abun), .by = "sample_id") %>%
  mutate(highlight = any(rela >= .1), .by = phylum_kraken2023)
```
...
arg_contigs_abf <- arg_contigs %>%
  ...
  summarise(abun = sum(`RPKM coverm`), .groups = "drop") %>%
  mutate(rela = abun / sum(abun), .by = "sample_id") %>%
  mutate(highlight = any(rela >= .1), .by = Antibiotic_Family)
...
shared_species <- #union(
  #filter(arg_contigs_sp, highlight) %>% pull(species_kraken2023) %>% unique,
...
shared_genus <- union(
  filter(arg_contigs_gen, highlight) %>% pull(genus_kraken2023) %>% unique,
...
shared_family <- union(
  filter(arg_contigs_fam, highlight) %>% pull(family_kraken2023) %>% unique,
...
shared_phylum <- union(
  filter(arg_contigs_phy, highlight) %>% pull(phylum_kraken2023) %>% unique,
...
saveRDS(color_abf, "cache/color_abf.rds")
```
