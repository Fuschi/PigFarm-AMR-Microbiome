...
p_bac_reads_species <- bac_reads_sp %>%
  mutate(species_fct = if_else(species %in% names(color_species), species, "Others")) %>%
...
p_bac_reads_genus <- bac_reads_gen %>%
  mutate(genus_fct = if_else(genus %in% names(color_genus), genus, "Others")) %>%
...
p_bac_reads_family <- bac_reads_fam %>%
  mutate(family_fct = if_else(family %in% names(color_family), family, "Others")) %>%
...
p_bac_reads_phylum <- bac_reads_phy %>%
  mutate(phylum_fct = if_else(phylum %in% names(color_phylum), phylum, "Others")) %>%
