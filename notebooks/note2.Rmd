...
df_composition_bac_sp_reads <- as_tibble(abun_bac_sp_reads, rownames = "sample_id") %>%
  ...
  ungroup() %>%
  group_by(genus) %>%
  mutate(top_genus = any(max(rela) >= .05 | genus == "Unknown")) %>%
  mutate(Genus = if_else(top_genus, genus, "Other")) %>%
  left_join(meta_sample, by = "sample_id") %>%
  mutate(Genus = if_else(Genus == "Unkown", "Unknown", Genus))
...
wanted_genus <- c(df_composition_bac_sp_reads$Genus, df_composition_arg_contigs$Genus) %>%
  unique()
wanted_genus <- wanted_genus[-which(wanted_genus %in% c("Other", "Unknown"))]
wanted_genus <- c(wanted_genus, c("Unknown","Other"))
...
## DEPENDENCIES BACTERIA GENERA VS ARG GENES

```{r, warning=FALSE, message=FALSE}
clr_bac_reads_genus <- abun_bac_sp_reads %>%
  as_tibble(rownames = "sample_id") %>%
  pivot_longer(-sample_id, names_to = "taxa_id", values_to = "abun") %>%
  left_join(as_tibble(taxa_bac_sp_reads, rownames = "taxa_id"), by = "taxa_id") %>%
  reframe(abun = sum(abun), .by = c("sample_id", "genus")) %>%
...
```{r, warning=FALSE, message=FALSE}
clr_arg_contigs_gene <- abun_arg_contigs_gene %>%
  zCompositions::cmultRepl(method = "CZM", suppress.print = TRUE, z.delete = FALSE) %>%
  vegan::decostand("clr")
```
...
ggpubr::ggarrange(
  ...
  tibble("mean_CLR_bac" = colMeans(clr_bac_reads_genus)) %>%
    ggplot(aes(mean_CLR_bac)) +
    geom_histogram(bins = 20, color = "darkblue", fill = "lightblue") +
    theme_bw(),

  tibble("mean_CLR_gene" = colMeans(clr_arg_contigs_gene)) %>%
    ggplot(aes(mean_CLR_gene)) +
    geom_histogram(bins = 20, color = "darkblue", fill = "lightblue") +
    theme_bw(),
